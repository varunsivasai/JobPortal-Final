...
public class JobSeekerProfileController {
    ...
    public String deleteProfile(HttpSession session) {
        ...
        if (profile != null) {
            try {
                Files.deleteIfExists(uploadDir.resolve(profile.getResumePath()));
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    /** Download resume */
    @GetMapping("/download")
    public ResponseEntity<?> downloadResume(HttpSession session) throws IOException {
        String username = (String) session.getAttribute("username");
        if (username == null) return ResponseEntity.badRequest().body("Please log in");

        RegisterJobSeeker user = userRepo.findByUsername(username);
        JobSeekerProfile profile = profileService.getByUser(user);

        if (profile == null) {
            return ResponseEntity.badRequest().body("No profile found. Please create a profile first.");
        }
